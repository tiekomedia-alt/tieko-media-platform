# Tieko Media Platform: Homepage Copy

**Document ID:** TMP-08  
**Version:** 0.1  
**Status:** Draft for approval  
**Date:** 17 September 2026  
**Owner:** Tieko Media Limited  
**Product lead:** David Oluwadamilola Vanteko  
**Route:** /  
**Parent documents:** TMP-00 to TMP-07  
**Repository:** tiekomedia-alt/tieko-media-platform

## 1. Page objective

The homepage must establish Tieko Media as an African digital intelligence and infrastructure company, explain its connected capabilities, direct visitors to the right service and generate qualified calls.

This document contains production copy. Text marked as a verification note or content placeholder must not appear publicly until the required evidence or asset is approved.

## 2. SEO metadata

### Recommended title

Tieko Media | Digital Intelligence and Infrastructure

### Recommended meta description

Tieko Media helps African businesses build stronger foundations, improve visibility, create useful content, manage information and make better decisions. Book a call.

### Canonical route

/

### Open Graph title

Build a Business People Can Find, Trust and Choose | Tieko Media

### Open Graph description

From media and content to compliance coordination, business intelligence, digital optimisation and records management, Tieko builds the systems behind credible growth.

### Primary topic

African digital intelligence and infrastructure company

### Supporting topics

- Digital growth company Nigeria
- Business visibility and infrastructure
- Media and content services Nigeria
- Business intelligence Africa
- Digital presence optimisation Nigeria

## 3. Global navigation copy

### Primary navigation

- Services
- Products
- Intelligence
- Work
- Insights
- About

### Primary navigation CTA

**Book a Call**

### Services menu groups

#### Build visibility

- Media Buying
- Digital Presence Optimisation
- Billboard Advertising

#### Build authority

- Content Creation
- Press Release Services

#### Build foundations

- Compliance
- Data and Records Management
- Business Automation & AI Systems

#### Build intelligence

- Intelligence Reports

## 4. Hero

### Eyebrow

**African digital intelligence and infrastructure**

### H1

**Build a business people can find, trust and choose.**

### Supporting copy

Tieko Media helps ambitious businesses and institutions build the foundations, visibility, content, intelligence and information systems they need to operate clearly and grow deliberately.

We research. We build. We amplify. We optimise.

### Primary CTA

**Book a Call**

### Secondary CTA

**Explore Our Services**

### Supporting trust line

**Built in Lagos. Useful anywhere.**

### Optional verified-rating line

**Rated 5 stars on Google**

Verification note: Do not publish this line until the exact Google Business Profile URL, current rating, review count and verification date are entered in the CMS.

### Hero image direction

Use an authentic African business, creative or institutional setting with purposeful human activity. The image should feel observed rather than staged.

The red Àmì Ribbon may curl through the composition to connect the headline, image and next section. It must not obscure the subject or copy.

Do not use:

- Generic stock-office handshakes
- Floating dashboards
- Artificial technology holograms
- Purple gradients
- Neon lighting
- Unverified customer logos
- Fake statistics

## 5. Problem statement

### Eyebrow

**Visibility needs a foundation**

### Heading

**Attention is not enough. Your business needs infrastructure.**

### Body copy

A campaign can bring people to your business. What they find next determines whether they trust it.

An incomplete business identity, a confusing website, inconsistent content, weak records, poor measurement or decisions made without evidence can turn attention into wasted effort.

Tieko connects the systems behind the public result. We help you build correctly, communicate clearly, reach deliberately and improve with evidence.

## 6. Capability framework

### Eyebrow

**One connected system**

### Heading

**Four capabilities. One stronger business.**

### Introduction

Growth problems rarely exist in isolation. Tieko brings together the foundations, systems, distribution and intelligence required to solve them properly.

### Capability 1

#### Establish

Build credible business and information foundations.

From business compliance coordination to records governance, we help organisations put essential structures in place before complexity grows.

**Related services:** Compliance, Data and Records Management

### Capability 2

#### Build

Create useful digital systems and owned assets.

We plan websites, content systems, digital products, assessments and conversion pathways designed around real business needs.

**Related services:** Digital Presence Optimisation, Content Creation, Products

### Capability 3

#### Amplify

Reach the right people with purpose.

We connect media buying, outdoor advertising, content and press distribution to a defined audience, objective and measurement plan.

**Related services:** Media Buying, Billboard Advertising, Content Creation, Press Release Services

### Capability 4

#### Protect and inform

Improve decisions, reputation and operational clarity.

We research markets, identify gaps, organise information and provide the intelligence needed to act with greater confidence.

**Related services:** Intelligence Reports, Digital Presence Optimisation, Data and Records Management

## 7. Services introduction

### Eyebrow

**What we do**

### Heading

**Services built around how businesses actually grow.**

### Supporting copy

You may need one focused service or a connected programme of work. We begin with the problem, identify what is missing and recommend a scope that makes commercial sense.

## 8. Eight service summaries

### 8.1 Media Buying

#### Heading

**Media Buying**

#### Copy

Plan and place advertising around a defined audience, campaign objective and measurement framework. Tieko coordinates channel selection, budget allocation, buying, optimisation and reporting across suitable media.

#### Link

**Explore Media Buying**

### 8.2 Compliance

#### Heading

**Compliance**

#### Copy

Establish and maintain important business, regulatory and intellectual-property foundations through a properly scoped process. Regulated legal services are delivered by David Vanteko through Vanteko & Associates where required.

#### Link

**Explore Compliance**

### 8.3 Digital Presence Optimisation

#### Heading

**Digital Presence Optimisation**

#### Copy

Improve how your business is found, understood, trusted and converted across your website, search engines, directories, content and emerging AI discovery systems.

#### Link

**Improve Your Digital Presence**

### 8.4 Content Creation

#### Heading

**Content Creation**

#### Copy

Build a repeatable content system that explains your value, develops authority and supports the customer journey. Strategy, production and distribution planning work together.

#### Link

**Explore Content Creation**

### 8.5 Intelligence Reports

#### Heading

**Intelligence Reports**

#### Copy

Turn fragmented market, competitor, search, audience and regulatory information into decision-ready intelligence. Commission research around a real commercial question.

#### Link

**Explore Intelligence Reports**

### 8.6 Billboard Advertising

#### Heading

**Billboard Advertising**

#### Copy

Plan outdoor campaigns around location, audience, format, creative visibility and verified placement. Tieko coordinates the campaign from assessment to reporting.

#### Link

**Explore Billboard Advertising**

### 8.7 Press Release Services

#### Heading

**Press Release Services**

#### Copy

Identify a credible news angle, prepare a clear release and coordinate suitable media outreach or distribution while distinguishing earned, sponsored and paid placement.

#### Link

**Explore Press Release Services**

### 8.8 Data and Records Management

#### Heading

**Data and Records Management**

#### Copy

Digitise, organise, classify, retain, retrieve and govern institutional records. Tieko also helps organisations discover useful intelligence within data they are authorised to process.

#### Link

**Explore Data and Records Management**

## 9. Service guidance CTA

### Heading

**Not sure which service fits the problem?**

### Copy

Tell us what is happening in your business. We will help you identify the immediate need, the dependencies around it and the most sensible next step.

### CTA

**Book a Call**

## 10. Digital Business Health Check

### Eyebrow

**Free business assessment**

### Heading

**How strong is your digital business foundation?**

### Body copy

A business can look active online and still have serious gaps underneath.

The Tieko Digital Business Health Check reviews eight areas that affect visibility, trust and growth:

1. Business Identity and Compliance
2. Website and Technical Foundation
3. Search Discoverability
4. Content and Brand Consistency
5. Customer Acquisition and Conversion
6. Data, Analytics and Measurement
7. Reputation and Trust
8. Privacy and Security Readiness

Complete the short assessment and receive an immediate indicative score, dimension results and three practical priorities.

Your basic result is not hidden behind an email form.

### Primary CTA

**Start the Free Health Check**

### Secondary CTA

**Learn About the Professional Audit**

### Disclaimer

The free result is an indicative self-assessment. It is not legal advice, a security certification or a professional audit.

## 11. Work introduction

### Eyebrow

**Selected work**

### Heading

**Strategy becomes valuable when it is built.**

### Body copy

We develop campaigns, digital infrastructure, content systems, research and original products. Our work is judged by whether it solves the intended problem, not by how many fashionable features it contains.

### Content placeholder

Feature two to four approved projects.

Each project must include:

- Client or product name
- Problem
- Tieko's role
- Approach
- Output
- Verified result where available
- Relevant service links

Do not publish placeholder client names or invented results.

### CTA

**View Our Work**

## 12. TeamShuffle product proof

### Eyebrow

**Built by Tieko**

### Heading

**We do not only advise. We build.**

### Body copy

TeamShuffle is a live collection of simple tools for creating fair groups and teams. It includes a random team generator, team balancer, wheel of names and virtual dice tools.

Building and growing TeamShuffle gives Tieko direct experience across product strategy, interface design, engineering coordination, search optimisation, content architecture, analytics and continuous improvement.

That same practical discipline shapes how we approach client work.

### Primary CTA

**Explore TeamShuffle**

### Secondary CTA

**View Our Products**

### External-link note

The TeamShuffle link must be clearly identified as opening the product website at teamshuffle.win.

## 13. Intelligence section

### Eyebrow

**Tieko Intelligence**

### Heading

**Make the decision with more than instinct.**

### Body copy

Markets move, competitors reposition, regulations change and customer demand leaves signals across multiple sources.

Tieko gathers, evaluates and organises those signals into research that helps organisations understand a market, examine an opportunity, assess a competitor or decide what to do next.

### Intelligence types

#### Market intelligence

Understand the structure, direction and commercial conditions of a market.

#### Competitor intelligence

Examine how relevant organisations position, communicate and compete.

#### Search and demand intelligence

Identify what people are looking for, how they express need and where demand may be forming.

#### Regulatory intelligence

Understand the rules, institutions and compliance considerations surrounding a decision.

### Primary CTA

**Explore Intelligence**

### Secondary CTA

**Commission a Report**

### Disclosure

Intelligence reports are paid unless a report is expressly marked as free.

## 14. Operating principles

### Eyebrow

**How we work**

### Heading

**Build the right thing before trying to scale it.**

### Principle 1

#### Strategy before noise

We define the problem, audience and intended outcome before choosing channels or deliverables.

### Principle 2

#### Systems before scale

Growth exposes weak foundations. We strengthen the underlying system before adding more traffic, content or complexity.

### Principle 3

#### Evidence before assumption

Research, measurement and real user behaviour should shape important decisions.

### Principle 4

#### Context before imitation

African businesses deserve work designed for their markets, constraints and ambitions, not copied templates with local names added.

### Principle 5

#### Clarity before complexity

A useful system should make the next decision easier. Technology and design must serve that purpose.

## 15. Why Tieko

### Heading

**More than a media agency.**

### Body copy

Media is one part of the work.

Tieko combines legal understanding, digital strategy, product thinking, research, content, data and distribution. This allows us to see the connections that isolated service providers often miss.

We can examine the foundation behind a campaign, the search problem behind a website, the records problem behind an institution or the intelligence gap behind a commercial decision.

The result is not more activity for its own sake. It is a clearer, stronger system for growth.

## 16. Trust module

### Recommended heading

**Trusted for thoughtful, practical work.**

### Verified-rating copy

**Tieko Media is rated 5 stars on Google, based on [verified review count] reviews.**

### Link

**Read Our Google Reviews**

### Verification requirements

The CMS must contain:

- Exact profile URL
- Current rating
- Review count
- Verification date
- Verification owner

If any requirement is missing, hide the numerical trust statement and use approved non-numerical proof instead.

Do not add self-serving organisation AggregateRating structured data.

## 17. Insights section

### Eyebrow

**Insights**

### Heading

**Useful thinking for people building in Africa.**

### Body copy

Practical analysis on digital infrastructure, media, search, content, compliance, information governance, African markets and product building.

### Initial featured-article placeholders

1. What Is a Digital Presence Audit?
2. Media Planning Versus Media Buying in Nigeria
3. How to Digitise Paper Records Without Creating Digital Disorder
4. What Is a Business Intelligence Report?

Only published articles should appear.

### CTA

**Read Our Insights**

## 18. About preview

### Eyebrow

**About Tieko**

### Heading

**Built in Lagos. Designed for serious growth.**

### Body copy

Tieko Media Limited is an African digital intelligence and infrastructure company led by David Oluwadamilola Vanteko.

We work with founders, companies, institutions and organisations that need more than temporary attention. We help them establish credible foundations, build useful systems, reach relevant audiences and make better-informed decisions.

### CTA

**About Tieko Media**

### Verification note

Confirm the final public company name, registration details and approved team-lead biography before publication.

## 19. Final conversion section

### Eyebrow

**Start with the real problem**

### Heading

**What does your business need to build next?**

### Body copy

Tell us what you are trying to achieve, what is getting in the way and what you have already attempted. We will use the first conversation to understand the problem and identify a practical next step.

### Primary CTA

**Book a Call**

### Supporting note

Booking a call does not create a solicitor-client relationship or guarantee that Tieko will accept an engagement.

## 20. Footer positioning copy

**Tieko Media is an African digital intelligence and infrastructure company. We research, build, amplify and optimise the systems businesses need to operate, compete and grow.**

### Footer groups

#### Services

- Media Buying
- Compliance
- Digital Presence Optimisation
- Content Creation
- Intelligence Reports
- Billboard Advertising
- Press Release Services
- Data and Records Management

#### Explore

- Products
- Intelligence
- Work
- Insights
- About
- Team
- Contact

#### Tools and actions

- Digital Business Health Check
- Book a Call
- Commission a Report
- TeamShuffle

#### Legal

- Privacy
- Cookies
- Terms
- Legal Disclaimer
- Accessibility

## 21. Image and media requirements

### Homepage image set

1. Hero image
2. One image representing African business activity
3. Selected-work images
4. TeamShuffle product image
5. Intelligence or research image
6. Optional founder or team image
7. Social sharing image

### Image rules

- Use authentic African context.
- Avoid generic office stock.
- Avoid over-polished artificial team scenes.
- Avoid AI artefacts.
- Obtain rights and releases.
- Provide meaningful alt text.
- Use empty alt text for decorative Àmì elements.
- Do not place important copy inside raster images.
- Optimise every asset for responsive delivery.

## 22. Suggested alt-text directions

Alt text must describe the final approved asset, not the planned concept.

Examples:

- Tieko Media team reviewing a campaign plan in Lagos
- TeamShuffle random team generator displayed on a mobile phone
- Researcher organising market and competitor findings
- Digitised institutional records arranged through a secure indexing system

Do not begin every description with image of.

## 23. Structured-data inputs

Homepage organisation markup may include only verified information:

- Legal name
- Approved trading name
- Canonical URL
- Logo URL
- Public email
- Public telephone
- Address or service area
- Founding information where approved
- Founder
- Social profiles
- Product relationships

Do not include:

- Self-serving AggregateRating
- Unverified awards
- Unapproved subsidiaries
- Confidential products
- Unsupported geographic offices

## 24. Internal-link map

| Homepage section | Destination |
|---|---|
| Hero secondary CTA | /services |
| Media Buying | /services/media-buying |
| Compliance | /services/compliance |
| Digital Presence Optimisation | /services/digital-presence-optimisation |
| Content Creation | /services/content-creation |
| Intelligence Reports | /services/intelligence-reports |
| Billboard Advertising | /services/billboard-advertising |
| Press Release Services | /services/press-release-services |
| Data and Records Management | /services/data-records-management |
| Health Check | /tools/digital-business-health-check |
| Work | /work |
| TeamShuffle | /products/teamshuffle and teamshuffle.win |
| Products | /products |
| Intelligence | /intelligence |
| Commission a Report | /services/intelligence-reports |
| Insights | /insights |
| About | /about |
| Book a Call | /book-a-call |

## 25. Mobile content priority

On small screens:

1. Category eyebrow
2. H1
3. Supporting copy
4. Book a Call
5. Explore Our Services
6. Problem statement
7. Four capabilities
8. Nine principal services
9. Health Check
10. Selected work
11. TeamShuffle
12. Intelligence
13. Operating principles
14. Trust
15. Insights
16. About
17. Final CTA

Requirements:

- Keep the primary CTA visible without a permanent obstructive bottom bar.
- Use a compact sticky header.
- Avoid horizontal service carousels that hide content.
- Do not reduce nine principal services to unlabeled icons.
- Keep body copy readable.
- Simplify Àmì Ribbon assets.
- Avoid excessive animation.
- Preserve the same links and meaning as desktop.

## 26. Analytics events

Track:

- homepage_book_call_click
- homepage_services_click
- homepage_service_select
- health_check_click
- work_click
- teamshuffle_click
- products_click
- intelligence_click
- commission_report_click
- insight_click
- about_click
- footer_book_call_click

Do not attach personal data to events.

## 27. Content still required before publication

- Google Business Profile URL
- Current Google review count
- Approved client case studies
- Approved measurable results
- Approved company registration wording
- Public telephone
- Public email
- Public location or service-area wording
- Approved founder biography
- Approved social profiles
- Final hero image
- Final TeamShuffle product image
- First published insight articles
- First public intelligence report or sample
- Booking provider and expected call duration

## 28. Homepage acceptance criteria

The homepage is ready for Stitch when:

1. The H1 is approved.
2. The nine principal services are correctly represented according to their approved implementation phase.
3. Compliance provider language is approved.
4. Data and Records Management language is approved.
5. TeamShuffle is approved as public product proof.
6. The Google rating is verified or hidden.
7. Case-study placeholders are replaced with approved work or omitted.
8. Every CTA has a destination.
9. Internal links are correct.
10. British English has been reviewed.
11. No em dash appears.
12. No unsupported claim appears.
13. Mobile content priority is approved.
14. The content can be understood without motion or imagery.
15. Stakeholder approval is recorded.

## 29. Recommended Stitch hand-off

The Stitch homepage prompt should include:

- Approved copy from this document
- Brand and Design System
- Àmì Ribbon references
- Anti-slop constraints
- Permanent sticky navigation
- Desktop, tablet and mobile requirements
- Content hierarchy
- Image direction
- Accessibility requirements
- Proof placeholders
- Prohibition on inventing logos, testimonials, statistics or dashboard data

Stitch must design around the approved content. It must not rewrite the strategy or manufacture proof.

## 30. Approval

**Product lead:** David Oluwadamilola Vanteko  
**Organisation:** Tieko Media Limited  
**Approval status:** Pending review
### Approved future homepage service addition

When the later service phase is approved, add **Business Automation & AI Systems** as the ninth principal service under **Build foundations**, linking to `/services/business-automation-ai-systems`.

The homepage presentation should describe practical workflow automation, customer response and follow-up, content-production systems, dashboards, AI assistants, knowledge systems, tool integrations and repetitive administrative-process automation. Do not publish the service card or link during Frontend Checkpoint 1.

Use the same canonical service name and anchor text wherever the service appears. The homepage and permanent navigation should maintain clear links to Services, Work, Intelligence, Insights, About, Store and Book a Call to support search-engine understanding without promising Google sitelinks. Do not add `SearchAction`, fake reviews, fabricated ratings or unsupported business claims.
